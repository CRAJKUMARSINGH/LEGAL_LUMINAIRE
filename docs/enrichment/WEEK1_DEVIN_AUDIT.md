...
5. **GitHub Actions `ci.yml` was failing on `main` before this PR** (last 3 `main` runs red): `corepack enable` without a pinned version resolves to pnpm 12.x, which turns "Ignored build scripts: esbuild, puppeteer" into a hard `ERR_PNPM_IGNORED_BUILDS` error. Netlify is unaffected because it pins `PNPM_VERSION=10`. Fixed in this PR by pinning CI to Node 22 + `corepack prepare pnpm@10 --activate` so CI and Netlify use the same toolchain.
