/**
 * PrecedentFitGate — visual component for fact-fit scoring display.
 * Shows fit level, score, holding, and application for each precedent.
 */
...
const fitConfig: Record<FitLevel, { label: string; color: string; icon: React.ElementType; border: string }> = {
  exact:    { label: "Exact Match — Primary Authority",          color: "text-green-800 bg-green-50",  icon: CheckCircle2,   border: "border-green-300" },
  analogous:{ label: "Analogous — Use with Qualification",       color: "text-blue-800 bg-blue-50",    icon: Info,           border: "border-blue-300" },
  weak:     { label: "Weak Fit — Supporting Only",               color: "text-amber-800 bg-amber-50",  icon: AlertTriangle,  border: "border-amber-300" },
  rejected: { label: "REJECTED — Factual Mismatch — DO NOT USE", color: "text-red-800 bg-red-50",      icon: XCircle,        border: "border-red-400" },
};

export default function PrecedentFitGate({ precedentName, citation, fitScore, fitLevel, fitReason, holding, application, children }: Props) {
  ...
  return (
    <div className={`border-2 rounded-xl overflow-hidden ${cfg.border}`}>
      <button onClick={() => setExpanded((p) => !p)} className={`w-full flex items-start gap-3 p-4 text-left ${cfg.color}`}>
      ...
      {fitLevel === "rejected" && (
        <div className="bg-red-50 border-t border-red-200 p-3">
          <p className="text-xs text-red-700 font-semibold">
            ...
            Using it as primary authority will weaken your argument. Reason: {fitReason}
          </p>
        </div>
      )}
    </div>
