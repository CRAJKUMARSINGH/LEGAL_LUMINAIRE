...
export function AccuracyBadge({ showDetails = false, size = "md" }: AccuracyBadgeProps) {
  ...
  const levelConfig = {
    CRITICAL: { color: "bg-green-100 text-green-800 border-green-300",  icon: ShieldCheck,    label: "CRITICAL ACCURACY", description: "10/10 - Ready for deployment" },
    HIGH:     { color: "bg-blue-100 text-blue-800 border-blue-300",     icon: CheckCircle,    label: "HIGH ACCURACY",     description: "8.0-9.4/10 - Good quality" },
    MEDIUM:   { color: "bg-yellow-100 text-yellow-800 border-yellow-300",icon: AlertTriangle,  label: "MEDIUM ACCURACY",   description: "6.0-7.9/10 - Needs improvement" },
    LOW:      { color: "bg-red-100 text-red-800 border-red-300",        icon: XCircle,        label: "LOW ACCURACY",      description: "Below 6.0/10 - Requires review" },
  };
