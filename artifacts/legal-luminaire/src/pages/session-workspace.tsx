...
// Static session data for Case 01 (Bridging logic from reference)
...
export default function SessionWorkspace() {
  ...
  const session = SESSIONS[id || ""] || SESSIONS["case01"]; // Default to Case 01 for hybrid preview

  return (
    <div className="flex h-[calc(100vh-3.5rem)] flex-col lg:flex-row overflow-hidden bg-background">
      {/* Left Panel */}
      <div className="w-full lg:w-80 flex-shrink-0 border-r border-border bg-card/30 flex flex-col overflow-y-auto">
        ...
        <div className="p-4 space-y-4 text-sm">
          ...
          <Card className="bg-amber-50 border-amber-200 shadow-none">
            <CardContent className="p-3 text-[11px] text-amber-800 leading-normal">
              <p className="font-bold mb-1 flex items-center gap-1.5 uppercase tracking-tighter">
                <AlertCircle className="w-3 h-3" /> Hybrid Engine Note
              </p>
              <p>This workspace integrates the Reference App's streaming draft engine with the Production Root architecture.</p>
            </CardContent>
          </Card>
        </div>
      </div>
      ...
      <div className="flex-1 p-6 overflow-y-auto bg-slate-50/30">
        {featureFlags.hybridDraftStreaming ? (
          <DraftGenerator sessionId={parseInt(session.id.replace("case", ""), 10) || 1} />
        ) : (
          <Card className="max-w-2xl mx-auto">
            <CardContent className="p-6 text-sm text-muted-foreground">
              Hybrid draft streaming is currently disabled by feature flag.
            </CardContent>
          </Card>
        )}
      </div>
